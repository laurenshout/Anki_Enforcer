...
class ProgressTracker:
    ...
    def _remaining_cards_by_selected_deck(
        ...
        # Prefer search-based counting first because it is more stable across Anki versions
        # and handles parent deck selections (including subdecks) consistently.
        from_search = self._remaining_from_search(mw, selected_ids)
        if from_search is not None:
            return from_search

        return self._remaining_from_scheduler_tree(mw, selected_ids)
    ...
    def _node_remaining_count(self, node: Any) -> int:
        return (
            ...
            + self._coerce_nonnegative_int(
                self._get(node, "learn_count", "learnCount", "learning_count", "learningCount", "learn")
            )
            + self._coerce_nonnegative_int(
    ...
    def _remaining_from_search(self, mw: Any, selected_ids: list[int]) -> Optional[dict[int, int]]:
        ...
        for deck_id in selected_ids:
            ...
            # Count cards left today in the selected deck:
            # - `is:due` covers due review/learning cards
            # - `is:new` covers new cards available today
            escaped_name = deck_name.replace('"', '\\"')
            query = f'deck:"{escaped_name}" (is:due OR is:new)'
            try:
